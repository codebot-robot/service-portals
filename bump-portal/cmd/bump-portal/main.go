...
func (t *gitTransport) RoundTrip(req *http.Request) (*http.Response, error) {
	log.Printf("--- Bump Portal (Git Proxy): Request Intercepted ---")
	log.Printf("URL: %s", req.URL.String())
	log.Printf("Method: %s", req.Method)

	// Basic git protocol awareness
	if strings.HasSuffix(req.URL.Path, "/info/refs") {
		log.Printf("Detected git info/refs request")
		// TODO: we could cache the refs response for a short duration
	} else if strings.HasSuffix(req.URL.Path, "/git-upload-pack") {
		log.Printf("Detected git-upload-pack request")
		// TODO: intercept and pull-through cache git objects
	}
	...
	log.Printf("--- Bump Portal: Response Intercepted ---")
